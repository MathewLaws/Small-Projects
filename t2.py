...
class Main:
    ...
    def weather(self):
        # title
        tk.Label(root, text="Weather", bg='#7CB9E8', font=font1).grid(row=0, column=0, sticky="W")
        ...
        print(self.weatherData["hourly"]["time"])
        ...
        #ax.set_xticks()
        #ax.set_xticklabels([i for i in range(0, len(self.weatherData["hourly"]["time"]), 12)], rotation=45)
        ...
        labels = [tk.Label(root, text=(f'{d[0]}: {d[1]}'), bg='#7CB9E8', font=font1)
        ...
        # back button
        tk.Button(root, text="Back", bg='#50C878', font=font1, command=lambda: (
            self.destroy_children(root), self.home())).grid(row=len(labels)+1, column=0, sticky="W")

    def air_quality(self):
        # title
        tk.Label(root, text="Air Quality", bg='#7CB9E8', font=font1).grid(row=0, column=0, sticky="W")
    ...
    def home(self):
        ...
        # company label
        company_label = tk.Label(root, text="Health Advice Group", bg='#7CB9E8', font=font1).grid(column=1, row=0, padx=(10, 0))

        # welcome text
        tk.Label(
            root, text=f"Welcome {self.user[1]}!", bg='#7CB9E8', font=font1).grid(column=0, row=1)

        # weather forecasting button
        tk.Button(
            root, text="Weather Forecasting", font=font1, bg='#50C878', command=lambda: (self.destroy_children(root), self.weather())).grid(column=0, row=2)

        # air quality button
        tk.Button(
            root, text="Air Quality Data", font=font1, bg='#50C878', command=lambda: (self.destroy_children(root), self.air_quality())).grid(column=0, row=3)

        # advice button
        tk.Button(root, text="Advice", font=font1, bg='#50C878', command=lambda: (
    ...
    def sign_up(self):
        ...
        # submit button
        tk.Button(root, text="Submit", font=font1, bg='#50C878', command=lambda: self.validation(
            username_entry.get(), password_entry.get(), False)).grid(column=0, row=3)

        # option to login if the user already has an account
        tk.Label(
            root, text="Already have an Account?", bg='#7CB9E8', font=font1).grid(column=0, row=4)
        tk.Button(root, bg='#50C878', font=font1, text="Login", command=lambda: (
    ...
    def login(self):
        ...
        # submit button
        tk.Button(root, font=font1, bg='#50C878', text="Submit", command=lambda: self.validation(
            username_entry.get(), password_entry.get(), True)).grid(column=0, row=3)

        # option to login if the user already has an account
        tk.Label(
            root, text="Dont have an Account?", bg='#7CB9E8', font=font1).grid(column=0, row=4)
        tk.Button(root, font=font1, bg='#50C878', text="Register", command=lambda: (
